...
import {addContribution, createIncidentFromExperience, startRunningIncident} from "../incidents/methods";
...
function createNewSpookyStorytime(){
  ...
  let storytimeCallback = function (sub) {
    ...
    Meteor.users.update({
      ...
      $set: {'profile.staticAffordances.participatedInSpookyStorytime': true}
    });
  ...
  let detectorIds = ["Dw9z8eTBvvF6EeqaR", "eqsBY5BBRZsFWfsS4", "Hewrfn8R87Z9EfjKh", "XHj47XpSWEE6Yrmm4", "3EML6ZvzjiKTK3Myy", "vj4M9wajY9HzgmM48"];
  let i = 0;
  _.forEach(places, (place) => {
    ...
    let det = {
      ...
      'variables': newVars,
      'rules': ['(' + CONSTANTS.DETECTORS[place].rules[0] + ' ) && !participatedInSpookyStorytime;']
    };
  ...
  let firstSentence = 'Something was happening in this small town of seemingly happy people.';

  let sendNotification = function (sub) {
    let uids = Submissions.find({iid: sub.iid}).fetch().map(function (x) {
      return x.uid;
    });
    notify(uids, sub.iid, 'Our spooky story is finally complete. Click here to read it!', '', '/apicustomresults/' + sub.iid + '/' + sub.eid);
  ...
  let exp = {
    ...
    resultsTemplate: 'storybook',
    contributionTypes: [{
      needName: 'pageOne', situation: {detector:"x7EgLErQx3qmiemqt", number: '1'},
      toPass: {
