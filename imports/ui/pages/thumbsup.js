...
Router.route('/participate/thumbsup/:_id', {
  template: 'thumbsup'
});
...
Template.thumbsup.onCreated(function() {
  this.submitting = new ReactiveVar(false);
  this.state = new ReactiveDict();
  const experiencesHandle = this.subscribe('experiences.byRoute', 'thumbsup');
  this.autorun(() => {
    if (experiencesHandle.ready()) {
      const experienceId = Experiences.findOne({route: 'thumbsup'})._id;
